...
void DoodleBug::move(Simulation* sim) {
    ...
    if (sim->isInBounds(newRow, newCol) && sim->isCellEmpty(newRow, newCol)) {
        sim->moveOrganism(row, col, newRow, newCol);
        if (sim->grid[newRow][newCol]->getSymbol() == 'Q') { sim->numQueens--; }
        if (sim->grid[newRow][newCol]->getSymbol() == 'o') { sim->numFemaleAnts--; }
        if (sim->grid[newRow][newCol]->getSymbol() == 'w') { sim->numMaleAnts--; }
        row = newRow;
        col = newCol;
    }
    moved = true;
